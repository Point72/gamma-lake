...
def read_table(feature_store, table_meta: dict, features: list[str], start, end, materialize=False) -> pl.LazyFrame | pl.DataFrame:
    ...
    if table_meta["signal_type"] == "as_of_feature":
        ...
        table_filter = pl.col(feature_store.primary_sort_key) <= end if end is not None and strategy == "backward" else pl.lit(True)
        table_filter &= pl.col(feature_store.primary_sort_key) >= start if start is not None and strategy == "forward" else pl.lit(True)

    table = (
        feature_store.io.scan_delta(feature_store.get_path(table_meta["table_addr"])).filter(table_filter).select(feature_store.sort_keys + features)
    )

    if as_of_params is not None:
